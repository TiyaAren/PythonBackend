...
def create_note(note: NoteCreate, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    return add_note(db, user_id, note)

@router.put("/{note_id}", response_model=NoteOut)
def update_note(note_id: str, note: NoteCreate, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
